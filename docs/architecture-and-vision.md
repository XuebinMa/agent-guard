...
- **Multi-Framework Adapters**: Official paths for LangChain-style and OpenAI-style integrations today, with AutoGen still treated as roadmap scope.
...
- **Framework Adapters**: Official Python/Node middleware for LangChain-style tools and OpenAI-style handlers, with AutoGen remaining a roadmap item.
- **Trust Tooling**: Standalone `guard-verify` CLI for non-technical auditors to verify signed receipts and generate doctor reports.
- **Transparency Dashboard**: Web-based report generated by `CapabilityDoctor` for easy stakeholder review.
- **Policy Signing Foundations**: Detached signature verification for policy loading is now part of the trust baseline rather than a pure future concept.

### Phase 2: Trust Moat (v0.4.0)
- **Policy Signing Hardening**: Promote signed policy loading from foundation capability to production-default guidance and lifecycle management.
