...
- **npm publishing uses trusted publishing (OIDC), like PyPI.** The npm job declares `environment: npm` and `id-token: write`, runs Node 24 for an npm CLI new enough to exchange the Actions OIDC token (>= 11.5.1, asserted in the job because an older CLI silently falls back to token auth), and passes no `NODE_AUTH_TOKEN`. Provenance is attached by default; do not add `--provenance`. This replaced a granular token with bypass-2FA set — a classic Publish token fails in CI with `EOTP` — which expires **2026-12-01** and whose mechanism npm ends in **January 2027**. That `NPM_TOKEN` secret is dead once a trusted publish succeeds and should be revoked then, not before.
- **`aws-lc-sys` does not cross-compile.** It reaches the SDK through `reqwest` -> `rustls` -> `aws-lc-rs`. Under the manylinux2014 aarch64 cross toolchain its C sources fail three ways at once (no `stdatomic.h`, unknown `-march=armv8.4-a+sha3`, undeclared `AT_HWCAP2`), which broke the Linux arm64 wheel in the first `v0.2.0` release run. The release workflow builds that wheel on a native `ubuntu-24.04-arm` runner instead; do not "fix" a cross failure here by chasing a newer cross toolchain.
