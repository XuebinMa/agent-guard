...
- **npm publishing runs on a token that expires twice.** The `NPM_TOKEN` secret is a granular access token scoped to `agent-guard-plugin` with "bypass 2FA" set, because a classic Publish token fails in CI with `EOTP` (it wants an interactive one-time password). That token expires **2026-12-01**, and npm ends bypass-2FA direct publishing in **January 2027**. Before the next release, move the npm job to npm Trusted Publishing (OIDC, the same mechanism the PyPI job already uses: `id-token: write`, no `NODE_AUTH_TOKEN`) and revoke the token. npm's own token form says as much.
