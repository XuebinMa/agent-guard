...
fn unwrap_command_wrappers<S: AsRef<str>>(tokens: &[S]) -> &[S] {
...
///
/// Walks segments split on `|` / `;` / `&&` / `||` / `&`, strips leading
/// `NAME=value` prefixes and transparent command-wrapper layers
/// (`sudo`/`env`/…) via `unwrap_command_wrappers`, then checks the command
/// word against `INLINE_CODE_INTERPRETERS`. Returns `Some((interpreter, flag))`
/// on the first hit.
